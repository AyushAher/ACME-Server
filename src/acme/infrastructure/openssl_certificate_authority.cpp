...
#include "acme/infrastructure/util/command_runner.h"
...
namespace acme::infrastructure
{
    ...
    std::string OpenSslCertificateAuthority::build_certificate_chain(
        const std::string &leaf_cert_path,
        const std::string &intermediate_cert_path) const
    {
        std::string chain;
        ...
        auto append_file = [&](const std::string &path)
        {
            ...
            {
                throw std::runtime_error("Certificate file not found: " + path);
            }
        ...
        append_file(leaf_cert_path);

        // Intermediate certificate second
        append_file(intermediate_cert_path);
        ...
        const domain::EabMapping &mapping) const
    {
        ...
        const auto issue_command =
            "openssl x509 -req "
            "-CA " +
            shell_escape(intermediate_cert) + " "
                                              "-CAkey " +
            shell_escape(intermediate_key) + " "
                                             "-set_serial 0x" +
            serial_hex + " "
                         "-sha256 "
                         "-days " +
            std::to_string(config_.valid_days) + " "
                                                 "-extfile " +
            shell_escape(ext_path) + " "
                                     "-extensions acme_server_cert "
                                     "-in " +
            shell_escape(csr_path) + " "
                                     "-out " +
        ...
        if (result.exit_code != 0)
        {
            return {
                .success = false,
                .certificate_pem_or_der = "",
                .raw_response = result.output,
                .error = "openssl x509 failed: " + result.output,
            };
        }
        ...
        if (cert_lines.empty())
        {
            return {
                ...
                .certificate_pem_or_der = "",
                .raw_response = "",
                .error = "openssl did not produce a certificate",
            };
        }
        ...
        // Append chain (Intermediate + Root)
        if (!config_.chain_file.empty() &&
            std::filesystem::exists(config_.chain_file))
        {
            const auto chain_lines = util::read_lines(config_.chain_file);
            ...
            if (chain_lines.empty())
            {
                return {
                    ...
                    .certificate_pem_or_der = "",
                    .raw_response = "",
                    .error = "Configured chain file is empty: " + config_.chain_file,
                };
            }
        ...
        else
        {
            return {
                ...
                .certificate_pem_or_der = "",
                .raw_response = "",
                .error = "Chain file not found: " + config_.chain_file,
            };
        }
        return {
            .success = true,
