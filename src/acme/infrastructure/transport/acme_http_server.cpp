    ...
    namespace acme::infrastructure::transport
    {
        ...
        namespace
        {
            ...
            std::string challenge_json(const domain::AcmeChallenge &challenge)
            {
                ...
                {
                    values["keyAuthorization"] = "\"" + infrastructure::util::json::escape(challenge.key_authorization) + "\"";
                }
                if (!challenge.error_detail.empty())
                {
                    values["error"] = "\"" + infrastructure::util::json::escape(challenge.error_detail) + "\"";
                }
            ...
            std::string authorization_json(const domain::AcmeAuthorization &authorization)
            {
                ...
                return infrastructure::util::json::object({{"status", "\"" + infrastructure::util::json::escape(authorization.status) + "\""},
            ...
            std::string order_json(const domain::AcmeOrder &order, const std::string &base_url)
            {
                ...
                return infrastructure::util::json::object({{"status", "\"" + infrastructure::util::json::escape(order.status) + "\""},
            ...
            {
                return infrastructure::util::json::object({{"status", "\"valid\""},
        ...
        std::string AcmeHttpServer::handle_request(const std::string &raw_request) const
        {
            ...
            try
            {
                ...
                {
                    return response(200, R"({"status":"ok"})", {{"Content-Type", "acme/application/json"}});
                }

                if (request.method == "GET")
                {
                    std::optional<std::string> token;
                    if (const auto well_known_token = path_param(request.path, "/.well-known/acme-challenge/");
                        well_known_token.has_value())
                    {
                        token = well_known_token;
                    }
                    else if (const auto http01_token = path_param(request.path, "/acme/http01/"); http01_token.has_value())
                    {
                        token = http01_token;
                    }
                    ...
                    if (token.has_value())
                    {
                        const auto challenge_response =
                            workflow_service_.get_http01_challenge_response(*token);
                        if (!challenge_response.has_value())
                        {
                            std::cout << "[acme] http01 miss token=" << *token << "\n";
                            return response(404, "not found", {{"Content-Type", "text/plain"}});
                        }
                        std::cout << "[acme] http01 serve token=" << *token
                                  << " body=" << *challenge_response << "\n";
                        return response(200, *challenge_response, {{"Content-Type", "text/plain"}});
                    }
                }
                ...
                if ((request.method == "HEAD" || request.method == "GET") && request.path == "/acme/newNonce")
                {
                    ...
                    return response(
                        ...
                        {"Content-Type", "acme/application/json"}});
                }

                if (request.method != "POST")
                {
                    return response(405, R"({"type":"malformed","detail":"method not allowed"})", {{"Content-Type", "acme/application/json"}});
                }
                ...
                if (const auto certificate_id = path_param(request.path, "/acme/certificate/"); certificate_id.has_value())
                {
                    ...
                    return response(200, certificate->pem_chain, {{"Content-Type", "acme/application/pem-certificate-chain"}, {"Replay-Nonce", nonce_service_.issue_nonce()}});
                }
            ...
            catch (const std::exception &ex)
            {
                return response(
                    ...
                    {{"Content-Type", "acme/application/json"}, {"Replay-Nonce", nonce_service_.issue_nonce()}});
            }
        }
